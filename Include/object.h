...
#else
#define _PyObject_HEAD_EXTRA // Py_TRACE_REFS(默认) 不存在时为空
#define _PyObject_EXTRA_INIT // Py_TRACE_REFS(默认) 不存在时为空
#endif

/* PyObject_HEAD 定义每个 PyObject 的初始段. */
...
/* 实际上没有任何东西被声明为 PyObject, 但是每一个指向Python对象的指针都可以转换为 PyObject*. 
 * 同样的每个指向可变Python对象的指针,都可以转换为 PyVarObject*.
 */
...
typedef struct {
    PyObject ob_base; // PyObject的基础部分
    Py_ssize_t ob_size; /* 变长部分的数目 */
} PyVarObject;
